...
setxkbmap latam
apt update -y && apt install -y whois ssh sshpass net-tools vim mysql-server
...
su -c 'mkdir ~/.ssh && cd ~/.ssh && ssh-keygen -b 4096 -t rsa -f id_rsa -q -N $string' $username
...
ipmask="172.20.4.100/24"
gw="172.20.4.1"
dns="172.20.2.26, 1.1.1.1, 8.8.8.8"
